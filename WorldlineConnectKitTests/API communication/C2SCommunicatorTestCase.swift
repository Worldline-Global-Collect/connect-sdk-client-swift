...
class C2SCommunicatorTestCase: XCTestCase {
    ...
    var applePayProduct: PaymentProduct!
    var googlePayProduct: PaymentProduct!
    ...
    override func setUp() {
        ...
        let applePaymentProductJSON = Data("""
        {
            ...
            "displayHints": {
                "displayOrder": 11,
                "label": "Apple Pay",
                "logo": "/templates/master/global/css/img/ppimages/pp_logo_302_v1.png"
            },
            "usesRedirectionTo3rdParty": false
        }
        """.utf8)

        applePayProduct = try? JSONDecoder().decode(PaymentProduct.self, from: applePaymentProductJSON)

        let googlePayProductJSON = Data("""
        {
            "fields" : [ {
                "dataRestrictions" : {
                    "isRequired" : true,
            ...
            "displayHints": {
                "displayOrder": 15,
                "label": "Google Pay",
                "logo": "/templates/master/global/css/img/ppimages/pp_logo_320_v1.png"
            },
            "usesRedirectionTo3rdParty": false
        }
        """.utf8)

        googlePayProduct = try? JSONDecoder().decode(PaymentProduct.self, from: googlePayProductJSON)
        ...
        configuration =
            C2SCommunicatorConfiguration(
                ...
                assetBaseURL: "https://ams1.sandbox.api-worldline.com/client/v1",
                appIdentifier: "",
                ipAddress: ""
            )
    ...
    func testApplePayAvailabilityWithApplePay() {
        stub(condition: isHost("ams1.sandbox.api-worldline.com")) { _ in
             let response = [
        ...
        let paymentProducts = BasicPaymentProducts()
        paymentProducts.paymentProducts.append(applePayProduct)
    ...
    func testApplePayProduct() {
        XCTAssertEqual(
            applePayProduct.identifier,
            SDKConstants.kApplePayIdentifier,
            "Received product id not as expected"
        )
        XCTAssertEqual(applePayProduct.paymentMethod, "mobile", "Received product paymentMethod not as expected")
        XCTAssertEqual(applePayProduct.displayHints.label, "Apple Pay", "Received product label not as expected")
        XCTAssertEqual(applePayProduct.displayHints.displayOrder, 11, "Received product displayOrder not as expected")

        guard let field = applePayProduct.fields.paymentProductFields.first else {
            XCTFail("Received product field not as expected")
            return
        }

        XCTAssertTrue(field.dataRestrictions.isRequired, "Received product field isRequired is not true")
        XCTAssertTrue(
            field.dataRestrictions.validators.validators.isEmpty,
            "Received product field validators is not empty"
        )

        XCTAssertNil(field.displayHints, "Received product field displayHints is not nil")
    }

    func testGooglePayProduct() {
        XCTAssertEqual(
            googlePayProduct.identifier,
            SDKConstants.kAndroidPayIdentifier,
            "Received product id not as expected"
        )
        XCTAssertEqual(googlePayProduct.paymentMethod, "mobile", "Received product paymentMethod not as expected")
        XCTAssertEqual(googlePayProduct.displayHints.label, "Google Pay", "Received product label not as expected")
        XCTAssertEqual(googlePayProduct.displayHints.displayOrder, 15, "Received product displayOrder not as expected")

        guard let field = googlePayProduct.fields.paymentProductFields.first else {
            XCTFail("Received product field not as expected")
            return
        }
    ...
    func testPaymentProductWithId() {
        ...
        communicator.paymentProduct(withIdentifier: "1", context: context, success: { (paymentProduct) in
            ...
            XCTAssertTrue(field.displayHints?.formElement.valueMapping.count ?? 0 > 0, "No Value map found.")
