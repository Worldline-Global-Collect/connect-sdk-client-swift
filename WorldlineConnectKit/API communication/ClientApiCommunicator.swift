...
internal struct ClientApiCommunicator {
    ...
    func privacyPolicy(
        forProduct paymentProductId: String?,
        locale: String?,
        success: @escaping (_ privacyPolicyResponse: PrivacyPolicyResponse) -> Void,
        failure: @escaping (_ error: Error) -> Void,
        apiFailure: @escaping (_ errorResponse: ApiErrorResponse) -> Void
    ) {
        ...
        if let paymentProductId {
            components.queryItems?.append(URLQueryItem(name: "paymentProductId", value: paymentProductId))
        }
        ...
        let queryString = components.queryItems?.isEmpty == false ? (components.string ?? "") : ""
        let URL = "\(baseURL)/\(customerId)/services/privacypolicy\(queryString)"
